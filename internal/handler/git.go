...
func (h *GitHandler) Register(r *gin.RouterGroup) {
	...
	g.POST("/status-v2", h.StatusV2)
	g.POST("/log", h.Log)
	g.POST("/diff", h.Diff)
	g.POST("/file-diff", h.FileDiff)
	g.POST("/file-diff-v2", h.FileDiffV2)
	g.POST("/show", h.Show)
	g.POST("/add", h.Add)
	g.POST("/reset", h.Reset)
	g.POST("/change-selection", h.ChangeSelection)
	g.POST("/apply-selection-v2", h.ApplySelectionV2)
	...
	g.POST("/conflict-details-v2", h.ConflictDetailsV2)
	g.POST("/conflict-resolve-v2", h.ConflictResolveV2)
